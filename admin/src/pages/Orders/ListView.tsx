...
const Orders = () => {
  ...
  const handleFetchOrders = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BASE_URI}/order/all`, {
        headers: {
  ...
  const deleteOrder = async (id: string) => {
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_BASE_URI}/order/${id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const validation: any = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      licenseImage: Yup.string().required("Please upload Order's license"),
    }),
    ...
    onSubmit: async (values) => {
      ...
      try {
        await axios.post(
          `${process.env.REACT_APP_BASE_URI}/crm/Order`,
          {
            ...newData,
          },
          {
            headers: {
              Authorization: `Bearer ${localStorage.getOrder("token")}`,
            },
          }
        );
        handleFetchOrders();
      ...
      },
      {
        ...
        cell: (cell: any) => (
          <Dropdown className="relative">
            ...
            >
              <li onClick={() => deleteOrder(cell.row.original._id)}>
                x
  ...
  const dowloadOrdersInExcel = async () => {
    const data = Orders.map((Order: any) => {
      return {
        ...
        City: Order.city,
        Price: Order.price,
        "Date and Time": moment(Order.createdAt).format("DD/MM/YYYY hh:mm A"),
      };
    });
  ...
  const filterSearchData = (e: any) => {
    ...
    const searchResult = OrderList.filter((Order: any) => {
  ...
  return (
    <React.Fragment>
          ...
          <div className="card" id="Orders">
            <div className="card-body">
              ...
              <div className="flex Orders-center">
                <h6 className="text-15 grow">Orders List</h6>
              </div>
      ...
      >
        ...
        >
          <Modal.Title className="text-16">{"Create Order"}</Modal.Title>
        </Modal.Header>
