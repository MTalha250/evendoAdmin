...
const Shops = () => {
  ...
  const handleFetchShops = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BASE_URI}/shop`, {
        headers: {
  ...
  const deleteShop = async (id: string) => {
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_BASE_URI}/shop/${id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const validation: any = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      licenseImage: Yup.string().required("Please upload Shop's license"),
    }),
    ...
    onSubmit: async (values) => {
      ...
      try {
        await axios.post(
          `${process.env.REACT_APP_BASE_URI}/crm/Shop`,
          {
            ...newData,
          },
          {
        ...
        handleFetchShops();
        toast.success("Shop made successfully!");
        toggle();
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: "Logo",
        accessorKey: "carDetails.vehicleMake",
      ...
      },
      {
        ...
        cell: (cell: any) => (
          <Dropdown className="relative">
            ...
            >
              <li onClick={() => deleteShop(cell.row.original._id)}>
                x
              ...
              {/* <li onClick={() => navigate(`/Shops/${cell.row.original._id}`)}>
  ...
  const dowloadShopsInExcel = async () => {
    const data = Shops.map((item: any) => {
      return {
        "Shop ID": item._id,
        Name: item.name,
  ...
  const filterSearchData = (e: any) => {
    ...
    const searchResult = ShopList.filter((item: any) => {
  ...
  return (
    <React.Fragment>
      <BreadCrumb title="Shops" pageTitle="Shops" />
      <ToastContainer closeButton={false} limit={1} />
      <div className="grid grid-cols-1 gap-x-5 xl:grid-cols-12">
        <div className="xl:col-span-12">
          <div className="card" id="Shops">
            <div className="card-body">
              <div className="flex gap-2">
                <div className="ml-auto">
                  <button
                    onClick={dowloadShopsInExcel}
                    type="button"
              ...
              <div className="flex items-center">
                <h6 className="text-15 grow">Shops List</h6>
              </div>
                      ...
                      <div className="flex gap-2 xl:justify-end">
                        <div>
                          <button
                            ...
                            onClick={toggle}
                          >
                            <span className="align-middle">Add Shop</span>
                          </button>
                        </div>
                      </div>
                    </div> */}
      ...
      >
        ...
        >
          <Modal.Title className="text-16">{"Create Shop"}</Modal.Title>
        </Modal.Header>
